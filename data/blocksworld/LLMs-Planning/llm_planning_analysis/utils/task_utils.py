...
def generate_plan_cot(planexecutor, data, give_response):
    ...
    if give_response:
        ...
        for index, i in enumerate(planexecutor.plan):
            ...
            preconds = planexecutor.get_action_preconditions(i)
            precondition_text = get_state_translation(preconds, DATA)
            action = get_action_text(i, DATA)
            
            plan_text += f"Since {precondition_text}. "
            plan_text += f"Thus, we take action {action}. "
            # plan_text += f"Resulting State: {get_state_translation(state, DATA)}\n"
            steps += 1
        plan_text += f"Since the goal is {GOAL}. "
        plan_text += "The goal conditions are satisfied.<|eot_id|>"
        PLAN += plan_text
    else:
        plan_text = "\n"
