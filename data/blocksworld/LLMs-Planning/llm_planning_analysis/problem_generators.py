...
class Instance_Generator():
    ...
    def add_existing_files_to_hash_set(self):
        em = []
        count = 0
        for i in os.listdir(f"./dataset/{self.data['domain_name']}"):
            try:
                f = open(f"./dataset/{self.data['domain_name']}/instance-{count}.pddl", "r")
            except:
                em.append(count)
                count+=1
                continue
        ...
        try:
            for i in os.listdir(f"./instances_/{self.data['domain_name']}"):
                f = open(f"./instances_/{self.data['domain_name']}/{i}", "r")
                pddl = f.read()
                to_add = self.convert_pddl(pddl)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--max_blocks', type=int, default=6, help='max number of blocks in blocksworld')
