...
class Evaluator:
    ...
    def response_processor(self, response: str):
        ...
        response_answer = response_answer.split("###")[0]
        response_answer = response_answer.split("answer is")[-1]
        # response = response.split('.\n')[-1].strip()
        response_answer = response_answer.lower()
        if ' proved' in response_answer or "yes" in response_answer:
            return True
        elif " not provable" in response_answer or "unknown" in response_answer:
            return -1
        elif " disproved" in response_answer or " no." in response_answer or " no " in response_answer or "\"no\"" in response_answer or "no" in response_answer:
            return False
        else:
            return None
