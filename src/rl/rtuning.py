...
from src.utils.utils import reward_processor
import numpy as np
...
class ScriptArguments:
    ...
    num_samples: Optional[int] = field(default=32, metadata={"help": "the number of samples"})
...
def build_dataset(ds):
    ...
    def tokenize(sample):
        ...
        elif script_args.task_name == 'blocksworld': # TODO: fix the bug
...
tokenizer = transformers.AutoTokenizer.from_pretrained(script_args.model_name, use_fast=True)
tokenizer.pad_token = tokenizer.eos_token
...
# model = transformers.AutoModelForCausalLM.from_pretrained(script_args.model_name)
tokenizer = transformers.AutoTokenizer.from_pretrained(script_args.model_name)
tokenizer.pad_token = tokenizer.eos_token
...
with torch.no_grad():
    for datum in pbar:
        ...
        # filter out the duplicated responses
        if script_args.task_name == 'proofwriter':
            rewards = reward_processor([r.split('<|eot_id|>')[0].replace('</s>', '').strip() for r in responses], 
        ...
        # randomly sample 4 responses based on the rewards without placement
        ...
        for idx, sr, res in zip(range(len(sampled_responses)), sampled_rewards, sampled_responses):
            ...
            for i in range(len(sr)):
                ...
                if sr[i] > 0:
                    ...
                    idk_n += 1
                    if "sorry" not in res[i].lower():
                        if "gsm" in script_args.task_name or "MATH" in script_args.task_name:
                            idk_res = res[i].split('\n####')[0] + "\nSorry, I am not sure if I answer the question correctly. There might be mistakes in my answer. \n#### No answer. <|eot_id|> "
                        elif "boardgameQA" in script_args.task_name:
                            idk_res = res[i].split("\nSo the statement")[0] + "\nSorry, I am not sure if I answer the question correctly. There might be mistakes in my answer. <|eot_id|> "
                        elif "blocksworld" in script_args.task_name:
                            idk_res = res[i].split("The goal conditions are satisfied.")[0] + \
                                "\nSorry, I am not sure if I answer the question correctly. There might be mistakes in my answer. <|eot_id|> "
                    else:
                        idk_res = res[i]
                    if script_args.task_name == 'proofwriter':
                        new_data_dict['cot'].append(idk_res)
                    elif script_args.task_name == 'blocksworld':
                        new_data_dict['reply'].append(idk_res)
                    elif script_args.task_name == 'gsm' or script_args.task_name == 'gsm_small' or \
                        'boardgameQA' in script_args.task_name or 'MATH' == script_args.task_name:
                        new_data_dict['completion'].append(idk_res)
                    else:
                        raise NotImplementedError
                    for key in gathered_datum:
                        if key not in ['cot', 'reply', 'completion']:
